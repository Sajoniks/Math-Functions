...
Expression ExpressionBuilder::BuildExpression(const std::string & Expr)
{
	...
	for (auto It = Result.cbegin(); It < Result.cend(); ++It)
	{
		...
		case '+': case '-': case '/': case '*': case '^':
		{
			...
			/*if (!Stack.empty() && Ch == '-' && (Stack.top()->GetSymbol() == "-" || Stack.top()->GetSymbol() =="#") || (Output.empty()))
			{
				Stack.push(new Function("#"));
				continue;
			}*/
			
			if (Ch == '-' && (bMayUnary || Stack.empty()))
			{
				Stack.push(new Function("#"));
				continue;
			}
			...
			if (!Stack.empty())
			{
				auto* Op = dynamic_cast<Operator*>(Stack.top());
	...
	return Expression(std::move(Output), Vars, Result);
}
...
double Expression::operator()(const std::vector<double>& Point)
{
	return GetValue(Point);
}

bool Expression::HasFunction(const std::string& FuncName) const
{
	auto TempQueue = RPN;
	while(!TempQueue.empty())
	{
		const auto Element = dynamic_cast<Function*>(TempQueue.front());
		TempQueue.pop();

		if (Element && Element->GetSymbol() == FuncName)
			return true;
	}
